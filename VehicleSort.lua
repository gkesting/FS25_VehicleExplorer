...
-- Author: sperrgebiet
-- Please see https://github.com/sperrgebiet/FS25_VehicleExplorer for additional information, credits, issues and everything else
...
--VehicleSort.dirtyState = false;						-- Used to check if we have to sync the order with g_currentMission.vehicleSystem.vehicles
...
function VehicleSort:onPreLoad(savegame)
	--FS25 No idea if that's the proper way, but it seems it works
...
function VehicleSort:update(dt)
    ...
    VehicleSort.registerActionEvents()
	...
	if VehicleSort.showVehicles then
		...
		if realVeh.getIsControlled and not realVeh:getIsControlled() then
			...
			if envTardis == nil or
				...
				VehicleSort:controlVehicle(realVeh);
...
function VehicleSort:initVS()
	...
	--VehicleSort.tPos.y = g_currentMission.hud.topNotification.y - g_currentMission.hud.topNotification.textOffsetY;  -- y Position of Textfield, originally hardcoded 0.9
	VehicleSort.tPos.y = 0.8
	VehicleSort.tPos.yOffset = g_currentMission.hud.topNotification.textOffsetY * 2;  --* 1.5; -- y Position offset for headings, originally hardcoded 0.007
	VehicleSort.tPos.size = g_currentMission.hud.topNotification.textSize * 1.2;  -- TextSize, originally hardcoded 0.018
	VehicleSort.tPos.sizeBig = VehicleSort.tPos.size * 1.2;
	VehicleSort.tPos.sizeSmall = VehicleSort.tPos.size * 0.8;
	VehicleSort.tPos.sizeIncr = g_currentMission.hud.topNotification.titleTextSize - g_currentMission.hud.topNotification.textSize; -- Text size increase for headings
	VehicleSort.tPos.spacing = VehicleSort.tPos.size / 2.5;  -- Spacing between lines, originally hardcoded 0.005
...
-- ToDo MP
	...
	VehicleSort.bg = createImageOverlay(Utils.getFilename("hud/VeExListBackground.png", VehicleSort.ModDirectory));
    VehicleSort.bgX = 0.5;
...
--	end
--
--	VehicleSort:dp(string.format('#newOrder {%d} - #g_currentMission.vehicleSystem.vehicles {%d}', #newOrder, #g_currentMission.vehicleSystem.vehicles), 'SyncSortedWithGame');
--	if #newOrder == #g_currentMission.vehicleSystem.vehicles then
--		VehicleSort.Sorted = newSorted;
--		g_currentMission.vehicleSystem.vehicles = newOrder;
--		-- Update tab order
...
--		
--		VehicleSort:dp('Write back of orderd vehicles to g_currentMission.vehicleSystem.vehicles');
--	end
--end
...
function VehicleSort:getInfoTexts(realId)
	...
	if veh ~= nil then
		...
		if (not VehicleSort:isTrain(realId)) or (not VehicleSort:isCrane(realId)) then
			...
			-- FS25 local level, capacity = VehicleStatus:getDieselLevel(realId)
			if level ~= nil and capacity ~= nil then
				line = g_i18n.texts.fillType_diesel .. ": " .. level .. "/" .. capacity .. " " .. g_i18n.texts.unit_liter;
				table.insert(texts, line);
			end
			
			--FS25 local level, capacity = VehicleStatus:getDefLevel(realId)
			if level ~= nil and capacity ~= nil then
...
function VehicleSort:easyTab(realVeh)
	...
	if realVeh ~= nil then
		...
		if #VehicleSort.easyTabTable == 1 then
			VehicleSort:controlVehicle(VehicleSort.easyTabTable[1]);
		elseif #VehicleSort.easyTabTable > 1 then
			VehicleSort:controlVehicle(VehicleSort.easyTabTable[2]);
...
function VehicleSort:controlVehicle(realVeh)
    VehicleSort:dp(string.format('Entering vehicle: {%s}', realVeh.configFileName), 'controlVehicle');
	--realVeh:interact();
	--g_currentMission:requestToEnterVehicle(realVeh);
     g_currentMission.playerSystem:getLocalPlayer():requestToEnterVehicle(realVeh);
end
